...
export default function App() {
  ...
  const [guestReferences, setGuestReferences] = useState<ArtReference[]>(() => {
    const item = loadGuestReference();
    return item ? [item] : [];
  });
  useEffect(() => {
  ...
  useEffect(() => {
    applyTheme(profile?.theme_key ?? "sage");
  }, [profile?.theme_key]);
  useEffect(() => {
          ...
          <h1>
            {language === "en" ? (
              <>
                Your ideas.
                <br />
                <em>Your characters.</em>
              </>
            ) : (
              <>
                Твои идеи.
                <br />
                <em>Твои персонажи.</em>
              </>
            )}
          </h1>
  ...
  const chooseAccount = async (email?: string) => {
  ...
  return (
    <>
      ...
      />
      {page === "home" && <Home profile={profile} usage={usage} onUsageChange={refreshUsage} onCreate={() => setPage("create")} onGallery={() => setPage("gallery")} onDiscover={() => setPage("discover")} />}{" "}
      {page === "create" && (
        <ReferenceForm
          key={session.user.id}
          progressOwner={session.user.id}
          onCreated={async () => {
            const isFirst = references.length === 0;
            setReferences(await loadReferences());
            await refreshUsage();
            setPage("gallery");
            if (isFirst) setCelebrateFirstReference(true);
          }}
        />
      )}{" "}
      {page === "gallery" && <Gallery references={references} setReferences={setReferences} onCreate={() => setPage("create")} onReward={refreshUsage} />} {page === "training" && <Training />}
      {page === "shop" && usage && <Shop usage={usage} onUpdated={refreshUsage} />} {page === "discover" && <Discover />}
      {page === "profile" && <ProfileSettings profile={profile} email={session.user.email ?? ""} onUpdated={setProfile} onChooseAccount={chooseAccount} />} {celebrateFirstReference && <FirstReferenceCelebration onClose={() => setCelebrateFirstReference(false)} />}{" "}
      {showOnboarding && (
        <OnboardingTutorial
...
type GuestProps = {
  ...
  setReferences: React.Dispatch<React.SetStateAction<ArtReference[]>>;
  onExit: () => void;
};
function GuestApp({ page, setPage, references, setReferences, onExit }: GuestProps) {
  return (
    <>
      <Header page={page} isGuest onNavigate={setPage} onSignOut={onExit} />
      {page === "create" ? (
        <ReferenceForm
          isGuest
          progressOwner="guest"
          onCreated={(reference) => {
            if (reference) setReferences([reference]);
            setPage("gallery");
          }}
        />
      ) : page === "gallery" ? (
        <Gallery isGuest references={references} setReferences={setReferences} onCreate={() => setPage("create")} onReward={() => undefined} />
      ) : page === "training" ? (
        <Training />
      ) : (
