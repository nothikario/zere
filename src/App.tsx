...
export default function App() {
  ...
  return <><Header page={page} username={profile.username} onNavigate={navigate} onSignOut={async () => { await supabase.auth.signOut(); setLanguageChosen(false); setPage('home'); }}/>{page === 'home' && <Home profile={profile} usage={usage} onUsageChange={async () => setUsage(await visitAndGetUsage())} onCreate={() => navigate('create')} onGallery={() => navigate('gallery')} onDiscover={() => navigate('discover')}/>} {page === 'create' && <ReferenceForm onCreated={async () => { setReferences(await loadReferences()); setUsage(await visitAndGetUsage()); navigate('gallery'); }}/>} {page === 'gallery' && <Gallery references={references} setReferences={setReferences} onCreate={() => navigate('create')} onReward={async () => setUsage(await visitAndGetUsage())}/>} {page === 'training' && <Training/>} {page === 'shop' && usage && <Shop usage={usage} onUpdated={async () => setUsage(await visitAndGetUsage())}/>} {page === 'discover' && <Discover/>} {page === 'profile' && <ProfileSettings profile={profile} onUpdated={setProfile}/>}</>;
}
