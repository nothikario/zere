...
export function AccountSecurity() {
  ...
  return <section className="security-grid"><form className="settings-card form" onSubmit={changeEmail}><h2>{en ? 'Email' : 'Электронная почта'}</h2><label><span>{en ? 'Registered email' : 'Почта аккаунта'}</span><input name="account-email" type="email" autoComplete="email" value={newEmail} onChange={(event) => setNewEmail(event.target.value)} required/></label><small>{en ? 'Confirmation emails will protect the change.' : 'Смену нужно будет подтвердить по почте.'}</small><button disabled={busy || newEmail === email}>{en ? 'Change email' : 'Изменить почту'}</button></form>
    <form className="settings-card form" onSubmit={changePassword}><h2>{en ? 'Password' : 'Пароль'}</h2>{codeMode ? <label><span>{en ? 'Code from email' : 'Код из письма'}</span><input name="password-reset-code" inputMode="numeric" autoComplete="one-time-code" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} required/></label> : <label><span>{en ? 'Current password' : 'Старый пароль'}</span><input name="current-password" type="password" autoComplete="current-password" value={oldPassword} onChange={(event) => setOldPassword(event.target.value)} required/></label>}<label><span>{en ? 'New password' : 'Новый пароль'}</span><input name="new-password" type="password" autoComplete="new-password" minLength={8} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required/></label><button disabled={busy}>{en ? 'Set new password' : 'Установить новый пароль'}</button><button type="button" className="text-button" disabled={busy} onClick={() => void (codeMode ? setCodeMode(false) : forgotPassword())}>{codeMode ? (en ? 'Use current password' : 'Использовать старый пароль') : (en ? 'I forgot my password' : 'Я не помню пароль')}</button></form>{message && <p className="settings-message security-message">{message}</p>}</section>;
}
