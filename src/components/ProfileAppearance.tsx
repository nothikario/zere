...
export function ProfileAppearance({ profile, onUpdated }: { profile: Profile; onUpdated: (profile: Profile) => void }) {
  ...
  return <form className="settings-card form" onSubmit={save}><h2>{en ? 'Profile and appearance' : 'Профиль и внешний вид'}</h2><div className="avatar-setting">{avatarUrl ? <img src={avatarUrl} alt=""/> : <span>{displayName[0]}</span>}<label className="avatar-upload">{en ? 'Choose avatar' : 'Выбрать аватар'}<input name="avatar" type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => void chooseAvatar(event.target.files?.[0])}/></label></div>
    <label><span>{en ? 'Unique username' : 'Уникальный никнейм'}</span><div className="username-input"><b>@</b><input name="username" value={username} disabled={!canChangeUsername} onChange={(event) => setUsername(event.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))}/></div><small>{canChangeUsername ? (en ? 'You can change it now.' : 'Сейчас его можно изменить.') : `${en ? 'Available after' : 'Можно изменить после'} ${nextChange.toLocaleDateString(en ? 'en-US' : 'ru-RU')}`}</small></label>
    <label><span>{en ? 'Display name' : 'Псевдоним'}</span><input name="display-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} maxLength={40}/></label>
