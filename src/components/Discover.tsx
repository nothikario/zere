...
export function Discover() {
  ...
  return <main className="page discover-page"><div className="eyebrow">СООБЩЕСТВО REFRI</div><h1>Найди <em>художника</em></h1><p className="lead">Все зарегистрированные художники собраны здесь.</p>
    ...
    {selected ? <section><button className="back-button" onClick={() => setSelected(null)}>← К результатам</button><div className="public-profile"><div className="avatar">{selected.display_name[0]}</div><div><h2>{selected.display_name}</h2><p>@{selected.username}</p></div></div>{stats && <div className="streak-card">🔥 Стрик: <b>{stats.streak} дн.</b><span>Референсы: {stats.references_count}/{stats.references_limit} · В день: {stats.daily_limit}</span></div>}<div className="public-grid">{references.map((item) => <article key={item.id}><PublicImage path={item.final_art_path || item.image_path} title={item.title}/><div><h3>{item.title}</h3><p>{item.theme} · {item.pose}</p></div></article>)}</div>{!references.length && <p className="empty">У этого пользователя пока нет референсов.</p>}</section>
