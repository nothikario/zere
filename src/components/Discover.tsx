...
export function Discover() {
  ...
  async function search(event: React.FormEvent) { event.preventDefault(); setPeople(query.trim() ? await searchProfiles(query) : []); setSelected(null); }
  async function open(profile: Profile) { setSelected(profile); setReferences(await loadPublicReferences(profile.user_id)); }
  return <main className="page discover-page"><div className="eyebrow">СООБЩЕСТВО REFRI</div><h1>Найди <em>художника</em></h1><form className="people-search" onSubmit={search}><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Введи никнейм, например @art_name"/><button>Найти</button></form>{selected ? <section><button className="back-button" onClick={() => setSelected(null)}>← К результатам</button><div className="public-profile"><div className="avatar">{selected.display_name[0]}</div><div><h2>{selected.display_name}</h2><p>@{selected.username}</p></div></div><div className="public-grid">{references.map((item) => <article key={item.id}><PublicImage path={item.final_art_path || item.image_path} title={item.title}/><div><h3>{item.title}</h3><p>{item.theme} · {item.pose}</p></div></article>)}</div>{!references.length && <p className="empty">У этого пользователя пока нет референсов.</p>}</section> : <div className="people-list">{people.map((person) => <button key={person.user_id} onClick={() => open(person)}><span className="avatar">{person.display_name[0]}</span><span><b>{person.display_name}</b><small>@{person.username}</small></span><i>→</i></button>)}</div>}</main>;
}
