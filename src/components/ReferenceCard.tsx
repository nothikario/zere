...
export function ReferenceCard({ reference, onDelete, onHide, onGenerated, onReward }: Props) {
  ...
  return <article className="reference-card">
    <div className="artwork">{imageUrl ? <img src={imageUrl} alt={reference.title}/> : <div className="artwork-empty"><span>✦</span><small>Здесь появится визуал</small></div>}{reference.final_art_path && <span className="final-badge">Итоговый рисунок</span>}</div>
    <div className="card-body"><div className="card-title"><h2>{reference.title}</h2><div className="card-controls"><button className="hide-reference" onClick={onHide}>{reference.is_hidden ? 'Вернуть' : 'Скрыть'}</button><button className="delete" onClick={onDelete} title="Удалить навсегда">×</button></div></div>
      ...
      <button className="generate" disabled={busy} onClick={generate}>{busy ? 'Создаём…' : reference.image_path ? '↻ Сгенерировать заново' : '✦ Сгенерировать картинку'}</button>
      <label className="upload-art">{uploading ? 'Проверяем рисунок…' : '＋ Прикрепить свой рисунок'}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={upload} disabled={uploading}/></label>{message && <p className="message">{message}</p>}
    </div>
  </article>;
}
