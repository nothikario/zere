...
export function ReferenceForm({ onCreated, isGuest = false }: { onCreated: () => void; isGuest?: boolean }) {
  ...
  const count = Number(values.peopleCount || 1);
  const flow = useMemo<FlowItem[]>(() => [...globalKeys.map((key) => ({ key })), ...(count === 1 ? characterKeys.map((key) => ({ key, characterIndex: 0 })) : Array.from({ length: count }, (_, characterIndex) => ({ key: 'gender' as WizardKey, characterIndex, multi: true }))), { key: 'pose' }], [count]);
  ...
  const form = current.multi ? <MultiCharacterStep character={characters[current.characterIndex!]} index={current.characterIndex!} onChange={(key, value) => updateCharacter(current.characterIndex!, key, value)} onLink={(key, value) => updateLink(current.characterIndex!, key, value)} onComment={(value) => updateComment(current.characterIndex!, value)}/> : <><ChoiceStep step={{ ...shownStep, options }} value={active[step.key]} comment={active.comments[step.key]} link={linkValue} restricted={isGuest} hideCustom={step.key === 'peopleCount'} onValue={(value) => update(step.key, value)} onComment={(value) => update(step.key, value, true)} onLink={changeLink}/>{step.key === 'style' && !isGuest && <label className="style-upload"><span>Фото своей работы для похожего стиля</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => setStyleExample(event.target.files?.[0])}/><small>{styleExample?.name ?? 'Необязательно'}</small></label>}</>;
