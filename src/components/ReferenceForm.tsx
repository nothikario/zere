...
export function ReferenceForm({ onCreated, isGuest = false }: { onCreated: () => void; isGuest?: boolean }) {
  ...
  const count = Number(values.peopleCount || 1);
  const flow = useMemo<FlowItem[]>(() => [...globalKeys.map((key) => ({ key })), ...Array.from({ length: count }, (_, characterIndex) => ({ key: 'gender' as WizardKey, characterIndex, multi: true })), { key: 'pose' }], [count]);
