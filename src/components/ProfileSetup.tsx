...
export function ProfileSetup({ userId, onReady }: { userId: string; onReady: (profile: Profile) => void }) {
  ...
  return <main className="profile-setup"><div className="brand-static">Refri<span>.</span></div><section className="card"><div className="eyebrow">ПОСЛЕДНИЙ ШАГ</div><h1>Как тебя называть?</h1><p>Никнейм уникальный — по нему тебя найдут другие художники.</p><form className="form" onSubmit={submit}><label><span>Никнейм</span><div className="username-input"><b>@</b><input value={username} onChange={(event) => setUsername(event.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))} placeholder="art_name" minLength={3} maxLength={20} required /></div><small>Его можно изменить один раз в 30 дней</small></label><label><span>Псевдоним</span><input value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="Например, Лунный художник" maxLength={40} required /><small>Он может повторяться и меняться свободно</small></label><button disabled={busy}>{busy ? 'Создаём…' : 'Перейти в Refri →'}</button></form>{message && <p className="message">{message}</p>}</section></main>;
}
